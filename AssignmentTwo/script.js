...
************/
// Directional Light
const directionalLight = new THREE.DirectionalLight(0x404040, 180)
...
const drawRedCube = (i, material) => {
    const cubeGeometry = new THREE.BoxGeometry(.75, .75, .75); // Adjust the size of the cube as needed
    const cube = new THREE.Mesh(cubeGeometry, material);
    cube.position.x = i - 10
    cube.position.z = (Math.random() - 0.5) * 10
    cube.position.y = (Math.random() - 0.5) * 10
